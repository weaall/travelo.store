...
export const authValidator = {
    signUp: [
        ...
        body("name").notEmpty().withMessage("이름을 입력해주세요").trim().isLength({ min: 2, max: 20 }).withMessage("이름은 2자에서 20자 사이어야 합니다."),
    ...
    signInByKakao: [
        body("id")
            .notEmpty().withMessage("카카오아이디를 입력해주세요.")
            .trim()
            .isNumeric().withMessage("카카오아이디의 형식이 올바르지 않습니다."),
    ],

    signInByNaver: [
        body("token")
            .notEmpty().withMessage("네이버토큰을 입력해주세요.")
            .trim().withMessage("네이버토큰의 형식이 올바르지 않습니다."),
    ]
};
